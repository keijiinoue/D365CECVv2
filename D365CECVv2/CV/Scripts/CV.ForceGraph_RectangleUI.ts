...
namespace CV {
    ...
    * @class
    */
    export class ForceGraph_RectangleUI {
        /**
        ...
        * @property
        */
        divForCards: d3.Selection<string>;
        /**
        ...
        */
        findNode(prefixedId: string): d3.layout.GraphNodeForce {
            for (let i in this.forceList.nodes) {
                if (this.forceList.nodes[i]["id"] == prefixedId) return this.forceList.nodes[i];
            }
        ...
        * @function
        */
        setForceOnTick(forceConnectionLine, forceConnectionMask, forceConnectionMaskToClick, forceConnectionDescription, forceConnectionRole1, forceConnectionRole2, forceNodeCard): void {
            this.force.on("tick", function () {
                ...
                forceConnectionRole1
                    .attr("x", function (d, i) {
                        return CV.ForceGraph_RectangleUI.UpdatePositionARole(forceConnectionRole1[0][i]
                            , forceConnectionLine[0][i].x1.baseVal.value
                            , forceConnectionLine[0][i].y1.baseVal.value
                            , forceConnectionLine[0][i].x2.baseVal.value
                            , forceConnectionLine[0][i].y2.baseVal.value, false).x;
                    })
                    .attr("y", function (d, i) {
                        return CV.ForceGraph_RectangleUI.UpdatePositionARole(forceConnectionRole1[0][i]
                            , forceConnectionLine[0][i].x1.baseVal.value
                            , forceConnectionLine[0][i].y1.baseVal.value
                            , forceConnectionLine[0][i].x2.baseVal.value
                            , forceConnectionLine[0][i].y2.baseVal.value, false).y;
                    });

                forceConnectionRole2
                    .attr("x", function (d, i) {
                        return CV.ForceGraph_RectangleUI.UpdatePositionARole(forceConnectionRole2[0][i]
                            , forceConnectionLine[0][i].x1.baseVal.value
                            , forceConnectionLine[0][i].y1.baseVal.value
                            , forceConnectionLine[0][i].x2.baseVal.value
                            , forceConnectionLine[0][i].y2.baseVal.value, true).x;
                    })
                    .attr("y", function (d, i) {
                        return CV.ForceGraph_RectangleUI.UpdatePositionARole(forceConnectionRole2[0][i]
                            , forceConnectionLine[0][i].x1.baseVal.value
                            , forceConnectionLine[0][i].y1.baseVal.value
                            , forceConnectionLine[0][i].x2.baseVal.value
                            , forceConnectionLine[0][i].y2.baseVal.value, true).y;
                    });
        ...
        */
        appendAndGetForceConnectionMask(): d3.Selection<d3.layout.GraphNodeForce> {
        ...
        */
        appendAndGetForceConnectionMaskToClick(): d3.Selection<d3.layout.GraphNodeForce> {
        ...
        * @function
        */
        updateForceConnectionMaskToClick(): void {
            var forceConnectionDescription = this.svg.selectAll("text.connectionDescription")
                .data(this.forceList.links);
            var forceConnectionMaskToClick = this.connectionMaskToClickVis.selectAll("a.connectionMaskToClick")
                .data(this.forceList.links);
        ...
        */
        appendAndGetForceConnectionDescription(): d3.Selection<d3.layout.GraphNodeForce> {
            var forceConnectionDescription = this.svg.selectAll("text.connectionDescription")
                .data(this.forceList.links);
        ...
        * @function
        */
        updateForceConnectionDescription(): void {
            var forceConnectionDescription = this.svg.selectAll("text.connectionDescription")
                .data(this.forceList.links);
        ...
        */
        connectionMaskToClickTouchPointerMouseStart(d, i): void {
            ...
            for (var l in d.connector.CrmLinkArray) {
                ...
                var desc = (crmLink.ConnectionType == CRMLinkTypeEnum.ManyToMany) ? "(N:N関係)" : crmLink.Description;
        ...
        * @function
        */
        refresh(nodeObj: { name: string; id: string; iconURL: string }): void {
        ...
        */
        constructor(elementId: string, connectionMaskToClickElementId: string) {
        ...
        */
        static UpdatePositionARole(connectionRole, X1, Y1, X2, Y2, reverse): Helper.Point {
            var returnPoint = new Helper.Point(0, 0);
            ...
            var contactPoint = new Helper.Point(0, 0);
            ...
            if (0.0 < angle && angle <= Math.PI / 2.0) {
                ...
                if (contactPoint.x == CardControl.CARD_WIDTH / 2.0)
            ...
            // 左上方向への向きの場合
                ...
                returnPoint.x = contactPoint.x - connectionRole.getBBox().width - 8.0;
                if (contactPoint.y == -CardControl.CARD_HEIGHT / 2.0)
                    returnPoint.y = contactPoint.y - 8.0;
                else
                    returnPoint.y = contactPoint.y - connectionRole.getBBox().height + 6.0;
            }
            // 右上方向への向きの場合 if (-Math.PI / 2.0 < angle && angle <= 0.0) と同じ意味である。
            else {
                returnPoint.x = contactPoint.x + 8.0;
                if (contactPoint.y == -CardControl.CARD_HEIGHT / 2.0)
                    returnPoint.y = contactPoint.y - 8.0;
                else if (contactPoint.x == CardControl.CARD_WIDTH / 2.0)
                    returnPoint.y = contactPoint.y - 8.0;
                else
                    returnPoint.y = contactPoint.y - connectionRole.getBBox().height + 6.0;
            }
        ...
        static GetCardContactXAndY(a, angle, contactPoint): void {
            // チェックする接点
            ...
            if (0.0 < angle && angle <= Math.PI / 2.0) {
                // 右辺の接点を調査
                tryContactY = (a != null) ? a * CardControl.CARD_WIDTH / 2.0 : null;

                // 下辺の接点を調査
                tryContactX = (a != null) ? ((a == 0.0) ? null : CardControl.CARD_HEIGHT / 2.0 / a) : 0.0;

                // 右辺の接点を採用できる状態か
                if (tryContactY != null && tryContactY <= CardControl.CARD_HEIGHT / 2.0) {
                    contactPoint.y = tryContactY;
                    contactPoint.x = CardControl.CARD_WIDTH / 2.0;
                }
                // 下辺の接点を採用できる状態か
                else {
                    contactPoint.y = CardControl.CARD_HEIGHT / 2.0;
            ...
            // 左上方向への向きの場合
                ...
                // 左辺の接点を採用できる状態か
                if (tryContactY != null && -CardControl.CARD_HEIGHT / 2.0 <= tryContactY) {
                    contactPoint.y = tryContactY;
                    contactPoint.x = -CardControl.CARD_WIDTH / 2.0;
                }
                // 上辺の接点を採用できる状態か
        ...
        getCanvasPosition(): Helper.Point {
            let x = parseInt(d3.select("#MyCanvasToDrag").attr("data-dragx"));
            let y = parseInt(d3.select("#MyCanvasToDrag").attr("data-dragy"));
        ...
        moveCanvasPosition(dx: number, dy: number): void {
            d3.select("#MyCardConnectionDiv")
                .style("left", () => {
                    let x = parseInt($("#MyCanvasToDrag").attr("data-dragx")) + dx;
                    $("#MyCanvasToDrag").attr("data-dragx", x);
                    return x + "px";
                })
                .style("top", () => {
                    let y = parseInt($("#MyCanvasToDrag").attr("data-dragy")) + dy;
