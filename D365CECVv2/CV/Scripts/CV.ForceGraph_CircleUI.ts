...
namespace CV {
    ...
    */
    export class ForceGraph_CircleUI {
        /**
        * d3 force layoutオブジェクト
        * @property
        */
        force: d3.layout.Force<d3.layout.force.Link<d3.layout.force.Node>, d3.layout.force.Node>;
        /**
        * force layout用 リスト。ノードのリストとリンク（ノードとノードを結ぶ線）のリストを内包する。
        * @property
        */
        forceList: NodesLinks;
        /**
        * カードを描画するためのSVG内のg要素
        * @property
        */
        svgGForCards: d3.Selection<string>;
        /**
        * つながりの線を描画するためのSVG内のg要素
        * @property
        */
        svgGForLines: d3.Selection<string>;
        /**
        * つながりの説明文を描画するためのSVG内のg要素
        * @property
        */
        svgGForConnectionDescriptions: d3.Selection<string>;
        /**
        * つながりロールを描画するためのSVG内のg要素
        * @property
        */
        svgGForConnectionRoles: d3.Selection<string>;
        /**
        * つながりの説明文をクリックできる領域(<a>)群を格納するエリア
        * @property
        */
        connectionMaskToClickVis: d3.Selection<string>;
        /**
        * svg
        * @property
        */
        svg: d3.Selection<string>;
        /**
        ...
        */
        ...
        /**
        * ドラッグが開始された位置
        * マウスクリックされただけ、タッチされただけのタイミングでも記録する。
        */
        dragStartedPoint: CV.Helper.Point;
        /**
        * 現在のイベント処理中のd3オブジェクト
        * @property
        */
        currentEventD3obj = null;
        /**
        ...
        */
        ...
        /**
        * Role1やRole2の表示位置を決めるための、サイズ "L" 用のカードの半径。ストロークの太さも考慮している。
        */
        static radius_for_role_l: number;
        /**
        * Role1やRole2の表示位置を決めるための、サイズ "M" 用のカードの半径。ストロークの太さも考慮している。
        */
        ...
        */
        addNode(nodeObj: { name: string; entityImage: string; id: string; iconURL: string; x: number; y: number; fixed: boolean; entityName: string }): void {
        ...
        */
        addLink(linkObj: { source: string; target: string; linkId: string; description: string; role1: string; role2: string; connector: ConnectorControl }): void {
            ...
            if (foundSource != null && foundTarget != null) {
                this.forceList.links.push({
                    source: foundSource,
                    target: foundTarget,
                    id: linkObj.linkId, // このidは、リンクが作成された最初に指定したCRMLinkのLinkIdを固定的に保持し続ける。
                    description: linkObj.description, // このdescriptionは後に変更される可能性がある。
                    role1: linkObj.role1, // このrole1は後に変更される可能性がある。
                    role2: linkObj.role2, // このrole2は後に変更される可能性がある。
        ...
        */
        setMultipleCRMConnetionFound(crmLink: CRMLink): void {
            var found = false;
        ...
        */
        findNode(id: string): d3.layout.GraphNodeForce {
            for (var i in this.forceList.nodes) {
                if (this.forceList.nodes[i]["id"] == id) return this.forceList.nodes[i];
            }
        ...
        setForceSizeAndStart(): void {
            ...
            if (CV.connectionViewer.config.SmallerSizeEnabled) {
                this.force.size([window.innerWidth, window.innerHeight])
                    .linkDistance((d: d3.layout.GraphNodeForce) => {
                        if (d.source.sizeType == "M" && d.target.sizeType == "M") return CV.ForceGraph_CircleUI.CARD_DISTANCE_MM;
                        else if (d.source.sizeType == "L" && d.target.sizeType == "L") return CV.ConnectionViewer.CARD_DISTANCE;
                        else return CV.ForceGraph_CircleUI.CARD_DISTANCE_LM;
                    })
                    .charge(-5000)
                    .start();
            } else {
        ...
        * @function
        */
        setForceOnTick(forceConnectionLine: d3.Selection<d3.layout.GraphNodeForce>,
            forceConnectionDescriptionG: d3.Selection<d3.layout.GraphNodeForce>,
            forceConnectionRole1: d3.Selection<d3.layout.GraphNodeForce>,
            forceConnectionRole2: d3.Selection<d3.layout.GraphNodeForce>,
            forceNodeCard): void {
            this.force.on("tick", function () {
                ...
                forceConnectionRole1
                    .each((d, index, outerIndex) => {
                        let textElem: any = forceConnectionRole1[outerIndex][index];
                        // UpdatePositionARole の戻り値はオブジェクト
                        //{ "position": Helper.Point, "text-anchor": string, "alignment-baseline": string }
                        let obj = CV.ForceGraph_CircleUI.UpdatePositionARole(
                            d.source.radius
                            , d.source.x
                            , d.source.y
                            , d.target.x
                ...
                forceConnectionRole2
                    .each((d, index, outerIndex) => {
                        let textElem: any = forceConnectionRole2[outerIndex][index];
                        let obj = CV.ForceGraph_CircleUI.UpdatePositionARole(
                            d.target.radius
                            , d.source.x
                            , d.source.y
        ...
        */
        appendAndGetForceConnectionLine(): d3.Selection<d3.layout.GraphNodeForce> {
        ...
        */
        appendAndGetForceConnectionDescriptionG(): d3.Selection<d3.layout.GraphNodeForce> {
            // d3のfilterの挙動が理解できない。事前にフィルター済みのデータを用意することにした。
            let hasDescriptionArray = this.forceList.links.filter((value, index, array) => {
                return !(value.connector.Description == null || value.connector.Description == "");
            });
            let forceConnectionDescriptionG = this.svgGForConnectionDescriptions.selectAll("g.connectionDescriptionG")
                .data(hasDescriptionArray);

            let connectionDescriptionG = forceConnectionDescriptionG
                .enter()
                .append("g")
                .classed("connectionDescriptionG", true);

            // マスク部分
            let rect = connectionDescriptionG.append("rect")
            ...
            connectionDescriptionG.append("text")
                ...
                .text((d) => { return d.connector.Description; })
                .on("touchstart", (d, i) => { CV.forceGraph.connectionMaskToClickTouchPointerMouseStart(d, i); }) // タッチ用
                .on("pointerdown", (d, i) => { CV.forceGraph.connectionMaskToClickTouchPointerMouseStart(d, i); }) // PCでのIE用
                .on("mousedown", (d, i) => { CV.forceGraph.connectionMaskToClickTouchPointerMouseStart(d, i); }); // PCでのChromeやFirefox用

            // マスクの位置とサイズを調整
            rect.each((d, index, outerIndex) => {
                let svgTextObj: any = d3.selectAll("text.connectionDescription")[outerIndex][index];
                let w = svgTextObj.getBBox().width;
                let h = svgTextObj.getBBox().height;

                let thisRectObj: any = d3.selectAll("rect.connectionMask")[outerIndex][index];
        ...
        */
        updateForceConnectionDescriptionG(): void {
            ...
            // つながりの説明文部分
            let forceConnectionDescriptionG = this.svgGForConnectionDescriptions.selectAll("text.connectionDescription")
                .data(hasDescriptionArray)
                .text((d) => { return d.description; });

            // マスクの位置とサイズを調整
            let forceRect = this.svgGForConnectionDescriptions.selectAll("rect.connectionMask")
            forceRect.each((d, index, outerIndex) => {
                let svgTextObj: any = d3.selectAll("text.connectionDescription")[outerIndex][index];
                let w = svgTextObj.getBBox().width;
                let h = svgTextObj.getBBox().height;
        ...
        */
        appendAndGetForceConnectionRole1(): d3.Selection<d3.layout.GraphNodeForce> {
            let linkRole1HasTextList = this.forceList.links.filter((value, index, array) => {
                return value.role1;
            });
        ...
        * @return つながり1を表すd3.Selection
        */
        updateForceConnectionRole1(): d3.Selection<d3.layout.GraphNodeForce> {
            let linkRole1HasTextList = this.forceList.links.filter((value, index, array) => {
                return value.role1;
            });
            let forceConnectionRole1 = this.svgGForConnectionRoles.selectAll("text.connectionRole1")
        ...
        */
        appendAndGetForceConnectionRole2(): d3.Selection<d3.layout.GraphNodeForce> {
            let linkRole2HasTextList = this.forceList.links.filter((value, index, array) => {
                return value.role2;
            });
        ...
        * @return つながり2を表すd3.Selection
        */
        updateForceConnectionRole2(): d3.Selection<d3.layout.GraphNodeForce> {
            let linkRole2HasTextList = this.forceList.links.filter((value, index, array) => {
                return value.role2;
            });
            let forceConnectionRole2 = this.svgGForConnectionRoles.selectAll("text.connectionRole2")
        ...
        */
        connectionMaskToClickTouchPointerMouseStart(d, i): void {
            ...
            let data = hasDescriptionArray[i];
            ...
            let html = '';
            for (let l in data.connector.CrmLinkArray) {
                let crmLink = data.connector.CrmLinkArray[l];
                let r1 = (crmLink.Record1DisplayRoleName) ? crmLink.Record1DisplayRoleName : "・";
                let r2 = (crmLink.Record2DisplayRoleName) ? crmLink.Record2DisplayRoleName : "・";
                let desc = (crmLink.LinkType == CRMLinkTypeEnum.ManyToMany) ? "(N:N関係)" : crmLink.Description;
        ...
        */
        touchPointerMouseStart(d): void {
            ...
            if (!CV.forceGraph.currentEventD3obj) {
                ...
                forceGraph.focusACardControl(d);
            }
        }
        /**
        ...
        */
        taphold(event): void {
            ...
            if (dx * dx + dy * dy < 100) { // 距離にして10px分以上動いているかどうかのチェック
                // ドラッグせずに、長押ししている
        ...
        */
        cardToggleFixed(d3obj): void {
            ///console.log("in cardToggleFixed() d3obj.fixed = ", d3obj.fixed);
            let cardCircle = $("#" + d3obj.id + " .circleInCard");
            if (!(d3obj.fixed & 1)) {
        ...
        * @function
        */
        refreshWhenAdd(nodeObj: { name: string; id: string; iconURL: string }): void {
            let forceConnectionLine = this.appendAndGetForceConnectionLine();
            let forceConnectionDescriptionG = this.appendAndGetForceConnectionDescriptionG();
            let forceConnectionRole1 = this.appendAndGetForceConnectionRole1();
            let forceConnectionRole2 = this.appendAndGetForceConnectionRole2();

            let forceNodeCard = this.svgGForCards.selectAll("g.circleCard")
                .data(this.forceList.nodes);

            let nodeEnter = forceNodeCard.enter()
                .append("g") // カードの全体。クラスは circleCard
                .attr("id", (d) => { return d.id; }) // d.idにはすでにprefix付き。
            ...
            nodeEnter
                ...
                .attr("xlink:href", (d) => { return "#MyDefCircle" + d.sizeType; }) // MyDefCircleL あるいは MyDefCircleM
                .attr("fill", "rgb(166,166,166)")
                .attr("stroke", (d) => { return CV.CardControl.getEntityColor(d.entityName); })
                .attr("filter", (d) => {
            ...
            nodeEnter
                ...
                .attr("y", -46)
                .attr("xlink:href", (d) => {
                    if (d.entityImage) return "data:image/png;base64," + d.entityImage;
                    else return d.iconURL;
                })
                .attr("cursor", "move")
                .attr("width", 32)
                .attr("height", 32)
            ...
            let nodeEnterText = nodeEnter
                .append("text")
                .classed("nameInCard", true)
                .attr("font-family", "Meiryo UI")
                .attr("font-size", "9pt")
                .attr("fill", "white")
                .attr("text-decoration", (d) => { return (d.sizeType == "L") ? "underline" : null; })
                .attr("cursor", (d) => { return (d.sizeType == "L") ? "pointer" : "move"; })
                .text((d) => { return d.name; }); // いったんここに文字列を置いて、横幅のチェックをする。複数行になる場合には、後からここの文字列は削除されてtextPathの中に書かれる。
            ...
            nodeEnterText.each((d, index, outerIndex) => {
                let thisSvgText: any = d3.selectAll("text.nameInCard")[outerIndex][index];
                let computedTextLength = thisSvgText.getComputedTextLength();
            ...
            // 一行に収まるSVG Textの処理
            nodeEnterText.filter((d) => { return (d.numberOfLines == 1); })
                .attr("x", 0)
                .attr("y", 4)
                .attr("font-family", "Meiryo UI")
                .attr("font-size", "9pt")
            ...
            nodeEnterText.filter((d) => { return !(d.numberOfLines == 1); })
                ...
                .text("")
                .append("textPath")
                .attr("xlink:href", (d) => { return "#MyDefTextPath" + d.sizeType + d.numberOfLines; }) // 次のいずれか。"MyDefTextPathL2", "MyDefTextPathL3", "MyDefTextPathM2", "MyDefTextPathM3"
                .text((d) => { return d.name; });
        ...
        refreshWhenUpdate(): void {
            // ID は、IDPrefix 付きで扱う。
            let focusedId = CV.IDPrefix + CV.forceGraph.currentFocusedCardID;
            let directlyConnectedId: string[] = []; // フォーカスしているカードに直接つながっているカードのIdを格納する。

            for (let i in this.forceList.links) {
                let link = this.forceList.links[i];
            ...
            let forceNodeCard = this.svgGForCards.selectAll("g.circleCard")
                ...
                .each((d, index, outerIndex) => {
                    ...
                    if (id == focusedId || 0 <= directlyConnectedId.indexOf(id)) {
                        ...
                        let previousSizeType = d.sizeType;
                        d.sizeType = "L";
            ...
            let nodeNameInCard = this.svgGForCards.selectAll("g.circleCard > text.nameInCard")
                .data(this.forceList.nodes)
                .each((d, index, outerIndex) => {
            ...
            // 一行に収まるSVG Textの処理
            nodeNameInCard.filter((d) => { return (d.numberOfLines == 1) ? true : false; })
                .attr("x", 0)
                .attr("y", 4)
                .attr("text-decoration", (d) => { return (d.sizeType == "L") ? "underline" : null; })
                .attr("cursor", (d) => { return (d.sizeType == "L") ? "pointer" : "move"; })
                .attr("text-anchor", "middle")
                .attr("onclick", (d) => { return (d.sizeType == "L") ? "CV.CardControl.OpenNewCRMFormWindow(this)" : null; })
                .text((d) => { return d.name; })
                .select("textPath")
                .style("visibility", "collapse");
            // 複数行になるSVG Textの処理
            nodeNameInCard.filter((d) => { return !(d.numberOfLines == 1); })
                .attr("x", 0)
                .attr("y", 0)
                .attr("text-decoration", (d) => { return (d.sizeType == "L") ? "underline" : null; })
                .attr("cursor", (d) => { return (d.sizeType == "L") ? "pointer" : "move"; })
         ...
         */
        static getNumberOfLines(computedTextLength: number, size: string): number {
            if (size == "L") {
        ...
        */
        constructor(svgGForCardsID: string, svgGForLinesID: string, svgGForConnectionDescriptions: string, svgGForConnectionRoles: string) {
        ...
        */
        static focusACardControlByCRMRecordId(id: string): void {
            if (CV.forceGraph.currentFocusedCardID != id) {
                ...
                if (d3.select("g#" + CV.IDPrefix + id + ".circleCard .circleInCard") != null) {
                    d3.select("g#" + CV.IDPrefix + id + ".circleCard .circleInCard").attr("fill", (d) => { return CV.CardControl.getEntityColor(d.entityName); });
                }
            }
        }
        /**
        ...
        * @param id {string} CRMレコードのid
        */
        static unfocusACardControlByCRMRecordId(id: string): void {
            //アンフォーカス
            if (d3.select("g#" + CV.IDPrefix + id + ".circleCard .circleInCard") != null) {
        ...
        */
        focusACardControl(d: any): void {
            ...
            if (d.id.indexOf(CV.IDPrefix) == 0) {
                ...
                if (CV.forceGraph.currentFocusedCardID == id) return;
        ...
        */
        static UpdatePositionARole(r: number, X1: number, Y1: number, X2: number, Y2: number, reverse: boolean
            , index?: number

        ): { "position": Helper.Point, "text-anchor": string, "dominant-baseline": string } {
            let position = new Helper.Point(0, 0);
            ...
            let dx = (!reverse) ? X2 - X1 : X1 - X2;
            let dy = (!reverse) ? Y2 - Y1 : Y1 - Y2;
        ...
        * 現在のキャンバスの位置を取得する。
        */
        getCanvasPosition(): Helper.Point {
            let x = parseInt(d3.select("#MyDragDropRect").attr("data-dragx"));
            let y = parseInt(d3.select("#MyDragDropRect").attr("data-dragy"));

            return new Helper.Point(x, y);
        }       /**
         ...
         * @param dy
         */
        moveCanvasPosition(dx: number, dy: number): void {
            d3.select("#MyDragDropG")
                .attr("transform", () => {
                    let x = parseInt($("#MyDragDropRect").attr("data-dragx")) + dx;
                    let y = parseInt($("#MyDragDropRect").attr("data-dragy")) + dy;
